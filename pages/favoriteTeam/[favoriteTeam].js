...
export async function getServerSideProps({query}) {
    const Query = query.favoriteTeam
    const pb = new PocketBase('https://pocketbaselucashunt.fly.dev');

    let teamQueryData = null

    try {
    teamQueryData = await pb.collection('team').getFirstListItem(`userID="${Query}"`, {
  });
} catch (error) {
  console.log(error)
}
...
teamQuery = teamQueryData.team
    ...
    dataPlayers = response.data.elements.filter((player) => player.team === parseInt(teamQuery))
    ...
    return (
      <>
        <div className=" h-screen flex justify-center items-center">
              ...
              <button className="bg-blue-500 hover:bg-blue-500/80 text-white py-4 px-12 rounded-lg font-bold text-lg"><Link href="/profile/selectteam"><a>Vælg et hold</a></Link></button>
